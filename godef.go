...
func checkMatch(f *ast.File, pos token.Pos) (match, error) {
	...
	if path == nil {
		return match{}, fmt.Errorf("can't find node enclosing position")
	}
	switch node := path[0].(type) {
	case *ast.Ident:
		field, ok := path[1].(*ast.Field)
		if !ok {
			return match{ident: node}, nil
		}
		return match{ident: node, wasEmbeddedField: len(field.Names) == 0}, nil
	case *ast.SelectorExpr:
		return match{ident: node.Sel}, nil
	default:
		return match{}, nil
	}
}
